...
data <- "ERR11437324_Altai-5.fa.mod.out.landscape.Div.Rname.tab"
...
ggsave(filename = "output.pdf", width = 10, height = 5, useDingbats = F)
...
# Optional: Generate TE landscape with age

# Substitution rate (Brassicaceae)
sub_rate <- 8.22e-9  # substitutions per site per year

# Calculate insertion age in million years
rep_table.m$age_Myr <- (rep_table.m$distance / (2 * sub_rate)) / 1e6

# Plot TE landscape using age instead of distance
ggplot(rep_table.m, aes(x = age_Myr, y = value / 1000000, fill = fam)) +
...
# Save the second PDF
ggsave(filename = "output_age.pdf", width = 10, height = 5, useDingbats = FALSE)
